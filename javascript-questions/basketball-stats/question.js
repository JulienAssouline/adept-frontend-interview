...
const totalScore = (pointsPerQuarter) => {
  return pointsPerQuarter.reduce((a, b) => {
    return a + b;
  }, 0);
};
...
const getAveragePoints = (players) => {
  const scores = Object.keys(players).map((d) => players[d].points);
  const scoresSum = totalScore(scores);
...
const getHighestScorers = (players, average) => {
  ...
  const result = Object.assign(
    ...Object.keys(players)
      .filter((key) => players[key].points > average)
      .map((key) => ({ [key]: { points: players[key].points } }))
  );
